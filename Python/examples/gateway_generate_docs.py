...
def get_header_section(endpoint, file_depth=1):
...
def main():
    ...
    if test_only or gateway.login(credentials['token']):
        ...
        output = get_header_section(current_endpoint, len(current_endpoint['documentation'].split('/')))

        # Get a reference to the current endpoint's request details.
        current_endpoint_request = current_endpoint['request']

        # Does the endpoint support any request query strings?
        if 'query' in current_endpoint_request:
            output += get_request_section(current_endpoint_request['query'])
        
        # Perform a GET request on the resource.
        #json_object = gateway.api_call('/' + current_endpoint_request['uri'])
        ...
        current_endpoint_response = current_endpoint['response']

        # We can override some known types.
        object_map = None
        if 'map_object' in current_endpoint_response:
            object_map = current_endpoint_response['map_object']
        ...
        if 'map_description' in current_endpoint_response:
            description_map = current_endpoint_response['map_description']
        ...
        count = 1
        for example_item in current_endpoint_request['examples']:
            # We skip calling the first example as this has already been queried above.
            if not test_only and count > 1:
                json_object = gateway.api_call('/' + current_endpoint_request['uri'] + ('?' + example_item['uri'] if 'uri' in example_item else ''))

            # Take the obtained JSON as an example.
            output += get_example_section(current_endpoint_request['uri'], example_item, json_object)
