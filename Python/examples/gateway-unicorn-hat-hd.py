...
if gateway.login(credentials['Token']):
    ...
    try:
        # Repeat forever unless the user press CTRL + C.
        while True:
            ...
            # Create a new image in memory that can fit all the text pixels (we scroll pictures wider than the screen, but there's no point storing a larger height).
            ...
            draw.text(xy=(0, -font_upper), text=line, fill=tuple([int(n * 255) for n in colorsys.hsv_to_rgb(int(w_now / args.maximum_watts_per_panel) / number_of_microinverters, 1.0, 1.0)]), font=font)
            ...
            while True:
                ...
                # Is there likely fresher data to retrieve from the Gateway? Stop scrolling.
                if next_reading_time <= time.time(): break
