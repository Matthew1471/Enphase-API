...
|Add or read a Phasor Measurement Unit (PMU).
...
|`/ivp/cellular/{EID}`
|Get specific cellular devices information.
...
==== IVP - Energy System (formerly "Ensemble") - DataRaw
...
|Get the aggregate State Of Charge (SOC) using Zigbee Security Control (SecCtrl) messages.
...
|`/ivp/meters`
|Get or set internal meter settings.
