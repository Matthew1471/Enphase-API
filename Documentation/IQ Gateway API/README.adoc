...
|`/ivp/meters/cts/{EID}`
|Update specific meter's (by EID) current CT clamp configuration.
...
|Get or set specific meter's (by EID) current CT clamp reverse polarity configuration.

|`link:IVP/Meters/CTS.adoc[CT Settings]`
|`/ivp/meters/cts`
|Get internal meter's current CT clamp settings.
